...
exports.testWebhook = exports.processWebhook = void 0;
const AppError_1 = __importDefault(require("../errors/AppError"));
const logger_1 = require("../utils/logger");
const CaktoIntegrationService_1 = __importDefault(require("../services/CaktoIntegrationService"));
const processWebhook = async (req, res) => {
    try {
        const payload = req.body;
        // Log do payload recebido
        logger_1.logger.info("=== CAKTO WEBHOOK RECEBIDO ===");
        ...
        logger_1.logger.info("Payload:", JSON.stringify(payload, null, 2));
        // Verificar se é um evento de pagamento aprovado
        if (payload.event !== "purchase_approved") {
            logger_1.logger.info(`Evento ${payload.event} ignorado - não é purchase_approved`);
        ...
        if (!validStatuses.includes(payload.data.status)) {
            logger_1.logger.info(`Status ${payload.data.status} ignorado - deve ser paid ou waiting_payment`);
            return res.status(200).json({ message: "Status inválido", received: true });
        }
        const { data } = payload;
        logger_1.logger.info("=== INICIANDO PROCESSAMENTO ===");
        logger_1.logger.info(`Cliente: ${data.customer.name} (${data.customer.email})`);
        logger_1.logger.info(`Valor: R$ ${data.amount}`);
        logger_1.logger.info(`Status: ${data.status}`);
        // Processar pagamento através do serviço
        const result = await CaktoIntegrationService_1.default.processPayment({
            amount: data.amount,
        ...
        logger_1.logger.info("Webhook Cakto processado com sucesso:", JSON.stringify(response, null, 2));
        return res.status(200).json(response);
    }
    catch (error) {
        logger_1.logger.error("Erro ao processar webhook Cakto:", error);
...
const testWebhook = async (req, res) => {
    logger_1.logger.info("=== TESTE WEBHOOK CAKTO ===");
    logger_1.logger.info("URL:", req.url);
    logger_1.logger.info("Method:", req.method);
    logger_1.logger.info("Headers:", JSON.stringify(req.headers, null, 2));
    ...
    if (req.method === "POST") {
        ...
        try {
            ...
            logger_1.logger.error("Erro no teste:", error);
            return res.status(200).json({
